...
public:
	...
	typename Tree<K, T>::Node* joinLR(class Tree<K, T>::Node*, class Tree<K, T>::Node*);
...
bool RND_Tree<K,T>::AddElemTree(K key, T data) {
	...
	if (this->root == nullptr) {//если дерево пусто
	...
	bool go_down = true, AddElemTree_into_root = true, loop = true; //ввод дополнительных переменных
	while ((go_down) && (AddElemTree_into_root)) {//пока не принято решение вставлять в корень и есть куда спускаться по дереву
		if (rand() < RAND_MAX / (temp->n + 1)) { //вероятность появления нового узла
			subroot = temp; //запоминаем в подкорень текущее положение
			AddElemTree_into_root = false; //сбрасываем ключ
		}
		else {
			if (t->key == temp->key) return false; //если такой корень уже существует, выводим сообщение об ошибке
			if (t->key < temp->key) { //выбираем по какой ветке идти
				this->count++;//увеличиваем счетчик пройденных шагов
				if (temp->left == nullptr) go_down = false;//если достигли конца ветки, то сбрасываем ключ
				else temp = temp->left; //идем по левой
			}
			if (t->key > temp->key) {//выбираем по какой ветке идти
				this->count++; //увеличиваем счетчик пройденных шагов
				if (temp->right == nullptr) go_down = false; //если достигли конца ветки, то сбрасываем ключ
	...
	while (1) { //вставка в лист
		this->count++;//увеличиваем счетчик пройденных шагов
		if (t->key == temp->key) return false;//если такой корень уже существует, выводим сообщение об ошибке
		if (t->key > temp->key) //выбираем подветку
			if (temp->right == nullptr) {//достигли конца ветки
			...
			if (temp->left == nullptr) {//достигли конца ветки
	...
	if (AddElemTree_into_root == NULL) {//если принято решение вставлять в корень, то производим вращения
		...
		while (loop) {
			...
			if (t->key > temp->key) {//левый поворот
				temp->right = t->left;
				if (t->left != nullptr) t->left->parent = temp;
				t->left = temp;
			}
			else {//правый поворот
				temp->left = t->right;
				if (t->right != nullptr) t->right->parent = temp;
				t->right = temp;
			}
			if (temp->parent != nullptr) {//корректировка связей родитель<->сын
			...
			if (temp->right != nullptr) temp->n = temp->n + temp->right->n;
			if (temp->left != nullptr) temp->n = temp->n + temp->left->n;
			t->n = 1;
			if (t->right != nullptr) t->n = t->n + t->right->n;
			if (t->left != nullptr) t->n = t->n + t->left->n;
		}
	}
	fixN(t);
	return true; //сообщаем об успешном выполнении операции
...
bool RND_Tree<K, T>::DelElemTree(K key) {
	...
	while (1) {
		this->count++; //увеличиваем счетчик пройденных элементов
		if (h == nullptr) return false; //если такого ключа нет
...
void RND_Tree<K, T>::fixN(class Tree<K, T>::Node* t) { //перерасчет балансировки
	if (t == nullptr) return; //если достигли конца ветви, выходим
	if (t->left == nullptr && t->right == nullptr) { t->n = 1; return; }
	else {
		if (t->left == nullptr) { t->n = t->right->n + 1; return; }
		if (t->right == nullptr) { t->n = t->left->n + 1; return; }
	}
...
void test_rand(int n) {
	...
	cout << "Размер дерева:" << tree.GetSizeTree() << endl;
	cout << "Для Рандомизированного дерева (до теста):" << endl;
	cout << "Размер дерева:" << tree1.GetSizeTree() << endl;
