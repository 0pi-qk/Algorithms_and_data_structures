...
public:
    ...
    virtual bool AddElemTree(Key, Data); //Включение данных с заданным ключом
    virtual bool DelElemTree(Key); //Удаление данных с заданным ключом
    ...
    public:
        ...
        Key& end() { //остановка прямого итератора на первый неопределённый элемент
            if (ptr->root == NULL) throw Ex;
            cur = ptr->root;
...
Data& Tree<Key, Data>::GetElemTree(Key key) { //получить значение по ключу
    ...
    if (tmp == NULL) throw Ex; //если не найдены данные с заданным ключем, то выводим исключение
...
bool Tree<Key, Data>::DelElemTree(Key key) { //удалить данные по ключу
    ...
    count = 0; //обнуляем счетчик пройденных элементов
    while (tmp != NULL && tmp->key != key) { //пока не пройдем все дерево и текущий ключ не равен искомому
        pred = tmp;//запоминаем предыдущий элемент
        if (key < tmp->key)//выбираем по какой ветке идти
            tmp = tmp->left;//спускаемся по левой ветке
        else
    ...
    if (tmp == NULL) //если такого элемента в дереве нет
        return false; //сообщаем об ошибке
    if (tmp->left != NULL && tmp->right != NULL) { //если удаление из середины
        tmp_0 = tmp;
        pred = tmp;
        tmp = tmp->right; //идем по правой ветке
        while (tmp->left != NULL) { //пока не достигнем конца левой ветки
            count++; //сделали еще один шаг по дереву
    ...
    if (tmp->key < pred->key)//выбираем по какой ветке идти
